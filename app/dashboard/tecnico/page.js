...
export default function TechnicianDashboard() {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      ...
      // LEER PERFIL COMPLETO (Incluyendo Score)
  ...
  const handleUploadCert = async (e) => {
      ...
      try {
          ...
          await addDoc(collection(db, "technicians", user.uid, "certifications"), {
              name: newCert.name,
              imageUrl: url,
              status: 'pending', // Esperando que Admin apruebe
  ...
  const acceptJob = async (order) => { if(!confirm("¿Aceptas el trabajo?")) return; try { await updateDoc(doc(db, "orders", order.id), { status: 'asignado', technicianId: user.uid, technicianPhone: user.phoneNumber }); alert("✅ Trabajo aceptado."); setSelectedOrder(null); setActiveTab('mis_trabajos'); } catch (error) { alert("Error"); } };
  const finishJob = async (order) => { if (!proofImageFile) { alert("⚠️ FOTO OBLIGATORIA"); return; } if(!confirm("¿Terminaste el trabajo?")) return; setIsUploading(true); try { const storageRef = ref(storage, `proofs/${order.id}/${Date.now()}_final.jpg`); const snapshot = await uploadBytes(storageRef, proofImageFile); const downloadURL = await getDownloadURL(snapshot.ref); await updateDoc(doc(db, "orders", order.id), { status: 'terminado', completedAt: new Date(), proofImageUrl: downloadURL }); alert("🏆 ¡Trabajo completado!"); setSelectedOrder(null); } catch (error) { alert("Error: " + error.message); } finally { setIsUploading(false); } };
  ...
  const sendMessage = async (e) => { e.preventDefault(); if (!newMessage.trim()) return; try { await addDoc(collection(db, "orders", selectedOrder.id, "messages"), { text: newMessage, senderId: user.uid, createdAt: serverTimestamp() }); setNewMessage(''); } catch (error) { console.error(error); } };
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-10">
      {/* HEADER TÉCNICO CON LOGO AGREGADO */}
      ...
      {/* MODAL MI PERFIL / CERTIFICACIONES */}
      {showProfile && (
                ...
                <div className="p-6">
                    ...
                    <div className="border-t pt-4">
                        ...
                        <form onSubmit={handleUploadCert} className="space-y-2">
                            <input type="text" placeholder="Nombre (Ej: Curso Electricidad INADEH)" className="w-full text-sm p-2 border rounded-lg" value={newCert.name} onChange={e => setNewCert({...newCert, name: e.target.value})} />
                            <input type="file" className="w-full text-sm text-slate-500" accept="image/*" onChange={e => setNewCert({...newCert, file: e.target.files[0]})} />
                            <button type="submit" disabled={uploadingCert} className="w-full bg-indigo-600 text-white py-2 rounded-lg font-bold text-sm hover:bg-indigo-700 disabled:bg-gray-400">
                                {uploadingCert ? 'Subiendo...' : 'Enviar para Validación'}
                            </button>
                        </form>
                    </div>
                </div>
            ...
            <div className="bg-white w-full max-w-md rounded-2xl overflow-hidden shadow-2xl relative animate-in fade-in zoom-in duration-200 max-h-[95vh] overflow-y-auto flex flex-col">
              {showChat ? (
                  ...
                  <>
                      ...
                      <div className="h-64 bg-gray-200 w-full relative shrink-0">
                          {selectedOrder.imageUrl ? <img src={selectedOrder.imageUrl} className="w-full h-full object-cover"/> : <div className="p-20 text-center text-gray-400">Sin foto</div>}
                      </div>
